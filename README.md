...
1. Use the Hammer icon to compile the code for you. You will note that the the project contains the `STM32L011D4PX_FLASH.ld` file. This is autogenerated by STM32CubeIDE from the `.ioc` file and is called a `linker-script`. This is necessary for the compiler to know what information about the target. Specifically, it tells the compiler the memory requirements of the application, including the size of the heap and the stack that will be needed to run the application. For the purposes of just programming the code into the SwitchBlox, it is not necessary to indulge in the finer details (unless you _really_ want to).

<p align="center">
  <img src="images/opening.png" alt="Opening" width="320" height="320">
</p>
